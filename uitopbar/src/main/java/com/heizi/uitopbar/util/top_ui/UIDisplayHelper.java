 ...
 * className: UIDisplayHelper
 * createTime: 2018/11/20 16:53
 */
public class UIDisplayHelper {
    ...
    public static int getScreenHeight(Context context) {
        ...
        if(PhoneDeviceHelper.isXiaomi() && xiaomiNavigationGestureEnabled(context)){
    ...
    public static int[] getRealScreenSize(Context context) {
        if (PhoneDeviceHelper.isEssentialPhone() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    ...
    public static int getUsefulScreenWidth(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isLandscape && PhoneDeviceHelper.isEssentialPhone()
        ...
        if (isLandscape) {
            ...
            if (PhoneDeviceHelper.isHuawei() && !UIDisplayHelper.huaweiIsNotchSetToShowInSetting(context)) {
    ...
    private static int getUsefulScreenHeight(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isPortrait && PhoneDeviceHelper.isEssentialPhone()
                    && Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                // https://arstechnica.com/gadgets/2017/09/essential-phone-review-impressive-for-a-new-company-but-not-competitive/
                // 这里说挖孔屏是状态栏高度的两倍
                result -= 2 * UIStatusBarHelper.getStatusbarHeight(context);
